...
HRESULT CBlackSkeleton::Initialize()
{
...
const int CBlackSkeleton::Progress()
{
	MonsterProgress();
	...
	if(m_tUnitInfo.iHp == 2)
	{
		if(!m_bChange)
		{
			...
			m_fJumpPow = 0.3f;
			m_tFrame.fFrame = 0.f;
			m_wstrObjKey = TEXT("Black_Skeleton_NoHead");
			m_bChange = true;
...
void CBlackSkeleton::Render()
{
	MonsterRender();
}

void CBlackSkeleton::Pattren()
{
	if(m_pBeatMgr->GetBeatBoomBoom())
	{
		if(m_wstrObjKey == TEXT("Black_Skeleton_NoHead"))
		{
			if(m_iCount == 1)
			{
				m_bJump = true;
